clc
clear
folderPath = 'C:\Users\mrszh\Desktop\code and data\NOISE\pi12\NOISE\pi12\GHZ1\';
...
D=[Z,X,Y]';
data01=D;
folderPath = 'C:\Users\mrszh\Desktop\code and data\NOISE\pi12\NOISE\pi12\GHZ2\';
    ...
    for b=1:3
        for c=1:3
            for x=1:2
                for y=1:2
                    for z=1:2
                        M3(:,:,z+(y-1)*2+(x-1)*4+(c-1)*8+(b-1)*24+(a-1)*72)=kron(M(:,:,x,a),kron(M(:,:,y,b),M4(:,:,z,c)));
                        MM3(z+(y-1)*2+(x-1)*4+(c-1)*8+(b-1)*24+(a-1)*72,:)=kron(m(:,:,x,a),kron(m(:,:,y,b),m4(:,:,z,c)))';
